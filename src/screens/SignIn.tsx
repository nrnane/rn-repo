...
const SignInForm = (props:IProps) => {
  ...
  const {cognitoLogin} = useSignUpHook();

  const handleButton = async () => {
    console.log('Entered phoneNumber ', phoneNumber);
    //setLoader(true);
    //await setItem('comp_phone', phoneNumber);
    /*dispatch({
    ...
    cognitoLogin(phoneNumber)
      .then((respData: any) => {
        console.log('cognitoLogin hook response ', respData);
       // setLoader(false);
        if (respData) {
          /*dispatch({
            type: actionTypeConstants.PROVIDER_DETAILS,
          ...
          props.navigation.navigate("OtpVerification", {isLogin: true});
        } else {
          //setErrorMsg('Invalid credentials');
          console.log('Login error12', respData);
        }
      })
      .catch(e => {
        console.log('cognitoLogin hook error ', e);
        if (e?.code === 'UserNotFoundException') {
          //setErrorMsg("User doesn't exist. Redirecting to singup page.");
          /*setTimeout(() => {
            dispatch({
              type: actionTypeConstants.PROVIDER_DETAILS,
              payload: {
  ...
  return (
    <FormControl>
      ...
      <Button variant="solid" onPress={handleButton} size="lg" mt={{ base: 5, md: 3 }}>
        SIGN IN
      </Button>
    </FormControl>
  );
